...
const permitType = [
  ...
  {value: 'to be late', label: 'To Be Late'},
  {value: 'leave early', label: 'Leave Early'},
];

const PermitCreate = ({navigation}: any) => {
  const {lat, lng} = useGetLocation();
  const [isPermitTypeVisible, setIsPermitTypeVisible] = useState(false);
  const [labelPermitType, setLabelPermitType] = useState<string>('');
  ...
  // MODAL DROPDOWN
  const changeModalVisibilityPermitType = (bool: boolean) => {
    setIsPermitTypeVisible(bool);
  };
  const setDataPermitType = (option: any, label: any) => {
    setLabelPermitType(label);
  };
  ...
  return (
    <View style={[layout.page]}>
      ...
      />
      <Header title="Permit Create" onPress={() => navigation.goBack()} />
      <ScrollView
        style={{flex: 1}}
        ...
        <View
          ...
          <Text style={[blackTextStyle]}>Permit Type</Text>
          <Dropdown
            visible={isPermitTypeVisible}
            data={permitType}
            label={labelPermitType}
            height={48}
            borderRadius={16}
            backgroundColor={greyThirdColor}
            setLabel={setLabelPermitType}
            setData={setDataPermitType}
            changeModalVisibility={changeModalVisibilityPermitType}
          />
          <Gap height={10} />
      ...
      <ModalConfirmation
        ...
        confirmText={`Request ${labelPermitType} permission`}
